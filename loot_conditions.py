                        ...
                        with open(file_path, "r", encoding="utf-8") as fh:
                            ...
                            for pool in json_data.get("pools", []):
                                for condition in pool.get("conditions", []):
                                    condition_values.append(condition.get("condition"))
                                    global_condition_values.append(condition.get("condition"))

                            print(f"Conditions in {file_path}: {condition_values}")
                    except Exception as e:
                        print(f"Error in file: {file_path}")
                        print(e)
                        raise
    print(f"Complete sest of conditions: {set(global_condition_values)}")
